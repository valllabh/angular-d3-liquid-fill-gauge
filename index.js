angular.module('D3LiquidFillGauge', [])
.directive('liquidFillGauge', function() {

  function uuid() {
    return 'd3lfg-xxyxxx'.replace(/[xy]/g, function(c) {
  ...
  return {
    restrict: 'C',
    template: '<svg id="{{uuid}}" width="100%" height="100%"></svg>',
    scope: {
      ngModel: '=',
      config: '=?config'
    },
    link: function(s, e, a){
      ...
      function update(){
        if(visual && !isNaN(s.ngModel)){
          visual.update(s.ngModel);
        }
      }

      s.$watch('ngModel', update);
      s.$watch('config', refresh);
